...
void __HOOKS__() {
    ...
    HOOK(libmain + O(0x3068c94), StartMatch); // required for AutoQueue: resolves _StartMatch, no offset change
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    // CALL(0); // REMOVED: kill.h dlsym hook is for cracked APK only (offset 0x5c43e0 targets cracked build, not Play Store). Causes memory corruption / Helium detection on official APK.
