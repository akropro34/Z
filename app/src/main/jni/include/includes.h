...
// ================== SCRAMBLED OFFSET VAULT ==================
// Holds decrypted (offset ^ XOR_KEY) pointers/offsets pulled down from the
// license server at login (see mod/keylogin.h). Indices are assigned by
// keylogin.h itself (Director, UserInfo, MainManager, MenuManager,
// VisualCue, StartMatch, Line2, Zero, Small, ...) — sized to 16 for headroom
// beyond the 10 currently in use (0-9).
...
// Set once keylogin.h's server auth flow succeeds (see menu.h's login
// screen); gates whether the real menu (DrawMenu) renders at all.
static bool logged_in = false;

// XOR key used to obscure the offsets in transit/at rest. Not a security
// boundary by itself (this is client-side obfuscation against casual
// inspection, not a secret store) — just needs to match whatever the
// server/build step used to encode the offsets it hands back.
#define XOR_KEY 0x9E3779B97F4A7C15ULL
